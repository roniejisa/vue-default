...
const defaultStore = computed(() => {
  return store.state.defaultStore
})
</script>
...
<template>
  <main class="p-24 text-center">
    <p class="font-bold">
      JS Framework:
      <a href="https://v3.vuejs.org/" target="_blank" class="font-normal italic underline text-green-500">{{ defaultStore.name }}</a>
      <span class="text-gray-400 px-2">+</span> <span class="text-sm font-normal italic">{{ defaultStore.arrayDefault.join(', ') }}</span>
    </p>

    <p class="font-bold">
      CSS Framework:
      <a href="https://tailwindcss.com/" target="_blank" class="font-normal italic underline text-green-500">{{ defaultStore.objectDefault.name }}</a>
    </p>
